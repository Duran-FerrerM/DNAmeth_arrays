...
qcReport(rgSet = RGset,pdf = "QC/qcReport.pdf",
         ...
         sampGroups = pData(RGset)$Sample_Group,
...
mdsPlot(dat = RGset,sampGroups = pData(RGset)$Sample_Group,sampNames = pData(RGset)$Sample_Name_Analysis)
dev.off()
...
pSignals <- ggplot(data.frame(qc),
  ...
  ggtitle(label = "Samples signal intensities",
          subtitle = paste0("Good=",table(qc$Signal.Intensity.ok)[["TRUE"]],
                            ", Bad=",table(qc$Signal.Intensity.ok)[["FALSE"]]
          )
  )+
...
mdsPlot(dat = RGset,sampGroups = qc$Signal.Intensity.ok,sampNames = rownames(qc))
dev.off()
invisible(gc())
...
Samples.Signal.Pvals.ok <- pData(object = grSet)$Sample_Name_Analysis[which(pData(object = grSet)$Signal.Intensity.ok & pData(object = grSet)$Mean.detection.pval<=Detection.Pval.Mean.Sample.cutoff)]
pData(object = grSet)$Sample.good.quality <- ifelse(pData(object = grSet)$Sample_Name_Analysis %in% Samples.Signal.Pvals.ok,TRUE,FALSE)
...
mdsPlot(dat = as.matrix(betas),sampGroups = pData(grSet)$Sample_Group,sampNames = pData(grSet)$Sample_Name_Analysis)
dev.off()
invisible(gc())

set.seed(6)
pdf(paste0("QC/mdsPlot.normalized.",Normalization.type,".Samples.good.quality.pdf"),height = 6,width = 6)
mdsPlot(dat = as.matrix(betas),sampGroups = pData(grSet)$Sample.good.quality,sampNames = pData(grSet)$Sample_Name_Analysis)
dev.off()
invisible(gc())
...
densityPlot(dat = as.matrix(betas[,pData(grSet)$Sample_Name_Analysis[which(pData(grSet)$Sample.good.quality)]]),
                sampGroups = pData(grSet)$Sample_Group[which(pData(grSet)$Sample.good.quality)],
...
data.table::fwrite(x = cbind(CpGs=rownames(betas),as.data.frame(betas)),
                   file = paste0("Normalized.Betas/",project.name,"_Betas_",Normalization.type,".txt"),sep = "\t")
data.table::fwrite(x = cbind(CpGs=rownames(betasNAs),as.data.frame(betasNAs)),
                   file = paste0("Normalized.Betas/",project.name,"_Betas.w.NA_",Normalization.type,".txt"),sep = "\t")
