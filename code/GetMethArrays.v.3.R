...
if(!interactive()){
  ...
  project <- "/media/mduran/Marti_SSD1/WORK/CLL_ERIC_Subsets/QC_ICGC.450k"
  
  #450k
  samp.sheet.path.450k <- "/media/mduran/Marti_SSD1/WORK/CLL_ERIC_Subsets/Data/SampleSheets_QC/ICGC.450k.csv"
  baseDir.450k <- "/media/mduran/Marti_SSD1/WORK/IDATS/IDATS_450k"#NULL#"path"
  
  #EPIC
  samp.sheet.path.EPIC <- NULL#"/media/mduran/Marti_SSD1/WORK/CLL_ERIC_Subsets/Data/SampleSheets_QC/ERIC.HCB.ICGC.EPIC.csv"
  baseDir.EPIC <- NULL#"/media/mduran/Marti_SSD1/WORK/IDATS/IDATS_850k/"
...
CreateRGsetFromIDATS <- function(IDATS.path = NULL,Sample.sheet.path = NULL){
  ...
  if(!all(c("Sample_Name_Analysis","Sample_Group_Analysis")%in%colnames(targets))){
    stop("You should specify 'Sample_Name_Analysis' and 'Sample_Group_Analysis' in you Sample Sheet")
  }
...
if(saveRGset){
  ...
  saveRDS(RGset,file=paste0("RData/RGset.rds"))
}
...
qcReport(rgSet = RGset,pdf = "QC/qcReport.pdf",
         ...
         sampGroups = factor(pData(RGset)$Sample_Group_Analysis,levels = unique(pData(RGset)$Sample_Group_Analysis)),
...
if(saveRGset.norm){
  saveRDS(RGset.norm,file=paste0("RData/RGset.norm.",Normalization.type,".rds"))
}
...
invisible(gc())
saveRDS(CpGs.detection.pvals,file = "RData/CpGs.detection.pvals.rds")
...
pMean.detection.pval <- ggplot(data.frame(pData(grSet)[order(pData(grSet)$Mean.detection.pval),]),
  ...
  facet_grid(.~Sample_Group_Analysis,scales = "free_x",space = "free_x")+
  xlab("Samples")+
...
if(Detect.Pval.pct.samples=="100"){
  ...
  CpGs.pval.ok <- names(which(rowMeans(CpGs.pval.ok,na.rm = T)>=(as.numeric(Detect.Pval.pct.samples)/100))) # Retain positions  do  have good pvalue in more than X fraction of the samples 
  length(CpGs.pval.ok)
}
...
if(length(CpGs.pval.ok)<length(rownames(grSet))){
  writeLines("Removing CpGs with bad signal/noise ratio intensity")
...
if( ncol(betas)<300){
  ...
  mdsPlot(dat = as.matrix(betas),
          sampGroups = factor(pData(grSet)$Sample_Group_Analysis,levels = unique(pData(grSet)$Sample_Group_Analysis)),
  ...
  densityPlot(dat = as.matrix(betas[,pData(grSet)$Sample_Name_Analysis[which(pData(grSet)$Sample.good.quality)]]),
              sampGroups = factor(pData(grSet)$Sample_Group_Analysis[which(pData(grSet)$Sample.good.quality)],levels = unique(pData(grSet)$Sample_Group_Analysis[which(pData(grSet)$Sample.good.quality)])),
